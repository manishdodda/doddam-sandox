CREATE OR REPLACE PROCEDURE COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.SP_PA__DATA_VALDTN_INS("P_SUB_AREA_NM" VARCHAR(16777216), "P_INTERFACE_NM" VARCHAR(16777216), "P_OBJECT_LAYER" VARCHAR(16777216), "P_DATABASE_NM" VARCHAR(16777216))
 ...
 try 
	{
		...
		var V_AUDIT_INS_SQ = `CALL COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.SP_PA__AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_PA__DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''Procedure'', ''R'',''NA'' )`; 
		
        v_return_value += "\\n V_AUDIT_INS_SQ: " + V_AUDIT_INS_SQ;
        var V_AUDIT_INS_ST = snowflake.createStatement( {sqlText: V_AUDIT_INS_SQ} ).execute();
		
		var V_SQ1 = `SELECT BATCH_ID FROM COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME)=UPPER(''`+ P_SUB_AREA_NM +`'') 
		AND LOAD_STATUS = ''R''`;
		...
		var V_SQ4 = `DELETE FROM  COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__ERROR_DTLS WHERE BATCH_ID=`+ V_BATCH_ID +` AND SUB_AREA_NM = ''` + P_SUB_AREA_NM +`''
		...
		if(V_RUN_ID_FIELD==''BATCH_ID'')
		{
			...
			var V_SQ6 =`INSERT INTO COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__ERROR_DTLS SELECT
		...
		var V_SQ7 =`SELECT COUNT(*) FROM COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__ERROR_DTLS 
		...
		var V_SQ_VW = `SELECT DISTINCT SRC_TABLENM, SRC_TABLE_SCHEMA
         FROM COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__ERROR_DTLS WHERE UPPER(INTERFACE_NM) = UPPER(''` + P_INTERFACE_NM + `'') AND UPPER(SUB_AREA_NM) = UPPER(''` + P_SUB_AREA_NM +`'')  AND BATCH_ID =` + V_BATCH_ID +`  AND UPPER(OBJECT_LAYER)= UPPER(''` + P_OBJECT_LAYER +`'')`;
						v_return_value += "\\n V_SQ_VW: " + V_SQ_VW;
			...
			while (V_ST_VW.next()){
				...
				SELECT distinct B.* , A.RULE_ID, A.RULE_TYPE, A.ERROR_MSG FROM COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__ERROR_DTLS A
		...
		var V_AUDIT_UPD_COMP_SQ = `CALL COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.SP_PA__AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_PA__DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''PROCEDURE'', ''C'',''NA'' )`;
	...
	catch (err)  
	{
	  ...
	  var V_AUDIT_UPD_FAIL_SQ = `CALL COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.SP_PA__AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_PA__DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''PROCEDURE'', ''F'',''`+output_return_value+`'')`;  

CREATE OR REPLACE PROCEDURE DB.SCH.SP_PRJ_DATA_VALDTN_INS("P_SUB_AREA_NM" VARCHAR(16777216), "P_INTERFACE_NM" VARCHAR(16777216), "P_OBJECT_LAYER" VARCHAR(16777216), "P_DATABASE_NM" VARCHAR(16777216))
...
-----------Initiating audit sp
		...
		var V_AUDIT_INS_SQ = `CALL DB.SCH.SP_PRJ_AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_PRJ_DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''Procedure'', ''R'',''NA'' )`; 
...
-----------Fetching Batch details
		...
		var V_SQ1 = `SELECT BATCH_ID FROM DB.SCH.PRJ_BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME)=UPPER(''`+ P_SUB_AREA_NM +`'') 
		AND LOAD_STATUS = ''R''`;
...
----------Fetching only those RULES which have IS_ACTIVE as Y
		...
		var V_SQ2 = `SELECT DISTINCT SRC_TABLE_SCHEMA, SRC_TABLENM, RUN_ID_FIELD FROM DB.SCH.DATA_VLDTN_RULE WHERE IS_ACTIVE = ''Y''
...
----------Deleting all error entries from error details for particular batch
		...
		var V_SQ4 = `DELETE FROM  DB.SCH.PRJ_ERROR_DTLS WHERE BATCH_ID=`+ V_BATCH_ID +` AND SUB_AREA_NM = ''` + P_SUB_AREA_NM +`''
        ...
        FROM DB.SCH.DATA_VLDTN_RULE WHERE IS_ACTIVE = ''Y'' AND 
...
---------Fetching error counts using rule sql in data validation
		...
		if(V_RUN_ID_FIELD==''BATCH_ID'')
		{
			...
			var V_SQ6 =`INSERT INTO DB.SCH.PRJ_ERROR_DTLS SELECT
			` + V_BATCH_ID +`,` + V_RULE_ID + `,''` + V_RULE_TYPE + `'', ''` + P_SUB_AREA_NM + `'', ''` + P_INTERFACE_NM + `'', ''` + P_OBJECT_LAYER + `'', ` +  V_UNIQUE_KEY  + `, ''` + V_SRC_TABLE_SCHEMA + `'',''` + V_SRC_TABLE_NAME + `'',''` + V_ERROR_MSG +`'', CURRENT_DATE(),  ''DODDAM''  FROM `+ V_SRC_TABLE +` WHERE ` + V_WHERE_CLAUSE +` AND BATCH_ID =` + V_BATCH_ID + ``;
		...
		else
		{
			...
			var V_SQ6 =`INSERT INTO DB.SCH.PRJ_ERROR_DTLS SELECT
			` + V_BATCH_ID +`,` + V_RULE_ID + `,''` + V_RULE_TYPE + `'', ''` + P_SUB_AREA_NM + `'', ''` + P_INTERFACE_NM + `'', ''` + P_OBJECT_LAYER + `'', ` +  V_UNIQUE_KEY  + `, ''` + V_SRC_TABLE_SCHEMA + `'',''` + V_SRC_TABLE_NAME + `'',''` + V_ERROR_MSG +`'', CURRENT_DATE(),  ''DODDAM''  FROM `+ V_SRC_TABLE +` WHERE ` + V_WHERE_CLAUSE +``;
...
-------------Fetching counts from error details table
		...
		var V_SQ7 =`SELECT COUNT(*) FROM DB.SCH.PRJ_ERROR_DTLS 
...
------------Comparing error details count vs counts from data validation having  rule_sql as where clause
		...
		var V_SQ_VW = `SELECT DISTINCT SRC_TABLENM, SRC_TABLE_SCHEMA
         FROM DB.SCH.PRJ_ERROR_DTLS WHERE UPPER(INTERFACE_NM) = UPPER(''` + P_INTERFACE_NM + `'') AND UPPER(SUB_AREA_NM) = UPPER(''` + P_SUB_AREA_NM +`'')  AND BATCH_ID =` + V_BATCH_ID +`  AND UPPER(OBJECT_LAYER)= UPPER(''` + P_OBJECT_LAYER +`'')`;
						v_return_value += "\\n V_SQ_VW: " + V_SQ_VW;
			...
			while (V_ST_VW.next()){
				...
				var V_SQ_KEY = `SELECT DISTINCT UNIQUE_KEY FROM DB.SCH.DATA_VLDTN_RULE WHERE UPPER(SRC_TABLENM) = UPPER(''` + V_NM +`'') `;
...
-------------Creating Error views				
				...
				SELECT distinct B.* , A.RULE_ID, A.RULE_TYPE, A.ERROR_MSG FROM DB.SCH.PRJ_ERROR_DTLS A
...
------------Calling audit sp	
		...
		var V_AUDIT_UPD_COMP_SQ = `CALL DB.SCH.SP_PRJ_AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_PRJ_DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''PROCEDURE'', ''C'',''NA'' )`;
	...
	catch (err)  
	{
	  ...
	  var V_AUDIT_UPD_FAIL_SQ = `CALL DB.SCH.SP_PRJ_AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_PRJ_DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''PROCEDURE'', ''F'',''`+output_return_value+`'')`;  

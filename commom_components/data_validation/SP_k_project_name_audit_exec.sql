...
try 
{
    ...
    var V_CONTROL_SCHEMA_NAME = "k_schema_name";
	
-------Fetching param value from parameter table	
    
	var V_SQ = `SELECT PARAM_VALUE FROM `+V_CONTROL_SCHEMA_NAME+`.k_project_name_PARAMETER_TABLE WHERE PARAM_NAME = ''CONTROL''`;
...
-------Fetching Active audit entry
	...
	var V_FETCH_BATCH_SQ = `SELECT BATCH_ID,APPLICATION_ID FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_AUDIT_DTLS WHERE AUDIT_ID = `+ P_AUDIT_ID +` AND UPPER(SUBJECT_AREA_NAME) = UPPER(''`+P_SUBJECT_AREA_NAME+`'') AND EXECUTION_STATUS = ''R''`;
...
-----------Fetching audit_id,APPLICATION_ID,BATCH_ID for active EXECUTION_STATUS and null REF_ID
            ...
            var V_SELECT_AUDITEXEC_SQ = `SELECT AUDIT_ID,APPLICATION_ID,BATCH_ID FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_AUDIT_EXEC_DTLS WHERE  UPPER(PROCESS_TYPE)= UPPER(''`+ P_PROCESS_TYPE +`'') AND  UPPER(SUB_PROCESS_NM)= UPPER(''`+ P_SUB_PROCESS_NM +`'') AND REF_ID is null AND  EXECUTION_STATUS=''R''`;
...
-----------Fetching audit_id,APPLICATION_ID,BATCH_ID for active EXECUTION_STATUS and not null REF_ID
            ...
            var V_SELECT_AUDITEXEC_SQ = `SELECT AUDIT_ID,APPLICATION_ID,BATCH_ID FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_AUDIT_EXEC_DTLS WHERE  UPPER(PROCESS_TYPE)= UPPER(''`+ P_PROCESS_TYPE +`'') AND  UPPER(SUB_PROCESS_NM)= UPPER(''`+ P_SUB_PROCESS_NM +`'') AND UPPER(REF_ID)=UPPER(''`+P_REF_ID+`'') AND  EXECUTION_STATUS=''R''`;
...
---------Fetching execution status from AUDIT_DTLS
			...
			var V_SELECT_AUDIT_SQ = `SELECT EXECUTION_STATUS FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_AUDIT_DTLS WHERE  AUDIT_ID = `+ V_AUDIT_ID +` AND  APPLICATION_ID =`+ V_AUDIT_EXEC_APPLICATION_ID +`   AND  UPPER(SUBJECT_AREA_NAME) = UPPER(''`+P_SUBJECT_AREA_NAME+`'') AND BATCH_ID =`+ V_AUDIT_EXEC_BATCH_ID +``;
...
-------------updating  audit entry for failed status and null ref_id
                        ...
                        var V_UPD_ARCH_RESTRT_DML = `UPDATE `+V_DATABASE_SCHEMA_NAME+`.k_project_name_AUDIT_EXEC_DTLS SET EXECUTION_END_TIME = CURRENT_TIMESTAMP(),EXECUTION_TIME_SECS = datediff(SECOND, EXECUTION_START_TIME, CURRENT_TIMESTAMP()), SRC_RECORD_COUNT = `+P_SRC_RECORD_COUNT+`
...
--------------inserting entry into  AUDIT_EXEC_DTLS
					...
					var V_INS_AUDIT_DML =`INSERT INTO `+V_DATABASE_SCHEMA_NAME+`.k_project_name_AUDIT_EXEC_DTLS(AUDIT_ID,APPLICATION_ID,BATCH_ID,REF_ID,SUB_PROCESS_NM,PROCESS_TYPE,EXECUTION_START_TIME,EXECUTION_STATUS,EXECUTION_STATUS_DESC,CREATED_DATE,CREATED_BY,LAST_UPDATE_DATE,LAST_UPDATE_BY) 
...
------Marking entry as F for failed status coming from parameter where ref_id null
			...
			var V_UPD_ARCH_DML = `UPDATE `+V_DATABASE_SCHEMA_NAME+`.k_project_name_AUDIT_EXEC_DTLS SET EXECUTION_END_TIME = CURRENT_TIMESTAMP(),EXECUTION_TIME_SECS = datediff(SECOND, EXECUTION_START_TIME, CURRENT_TIMESTAMP())
...
-----Marking entry as C for Complete status coming from parameterwhere ref_id is null
			...
			var V_UPD_ARCH_DML = `UPDATE `+V_DATABASE_SCHEMA_NAME+`.k_project_name_AUDIT_EXEC_DTLS SET EXECUTION_END_TIME = CURRENT_TIMESTAMP(),EXECUTION_TIME_SECS = datediff(SECOND, EXECUTION_START_TIME, CURRENT_TIMESTAMP()),SRC_RECORD_COUNT = `+P_SRC_RECORD_COUNT+`,TGT_RECORD_COUNT = `+P_TGT_RECORD_COUNT+`

CREATE OR REPLACE PROCEDURE COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.SP_k_proj_nm_DATA_VALDTN_UPD("P_SUB_AREA_NM" VARCHAR(16777216), "P_INTERFACE_NM" VARCHAR(16777216), "P_OBJECT_LAYER" VARCHAR(16777216), "P_DATABASE_NM" VARCHAR(16777216))
...
----------Selecting active batch
		...
		var SQ_BATCH_ID = `SELECT BATCH_ID FROM COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME)=UPPER(''`+ P_SUB_AREA_NM +`'') AND LOAD_STATUS=''R''`;
		//return SQ_BATCH_ID;
...
-----------Selecting all active data validation rules i.e where IS_ACTIVE = Y
			
		var SQ_TABLE_NM = `select distinct SRC_TABLENM FROM COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.DATA_VLDTN_RULE WHERE IS_ACTIVE = ''Y'' AND UPPER(INTERFACE_NM) = UPPER(''`+ P_INTERFACE_NM +`'') AND UPPER(SUB_AREA_NM) = UPPER(''`+ P_SUB_AREA_NM +`'') AND UPPER(OBJECT_LAYER)= UPPER(''`+ P_OBJECT_LAYER +`'')`;
		//return SQ_TABLE_NM;
...
------------Fetching schema and unique key to mark error records as Y
			
		var SQ_SCHEMA_NM = `SELECT distinct SRC_TABLE_SCHEMA, UNIQUE_KEY FROM COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.DATA_VLDTN_RULE WHERE  UPPER(INTERFACE_NM)=UPPER(''`+ P_INTERFACE_NM +`'') AND UPPER(SRC_TABLENM) = UPPER(''`+ V_TABLE_NAME +`'')`;
		//return SQ_SCHEMA_NM;
...
---------Updating error records as Y
		
		var SQ_ERR_UPD_STATEMENT_Y = `UPDATE `+ V_TABLE_NM +` SET ERROR_FLAG =''Y'' WHERE `+ V_SRC_UNIQUE_KEY +` in (Select distinct SRC_UNIQUE_KEY from COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__ERROR_DTLS where BATCH_ID =`+ V_BATCH_ID +` AND UPPER(SUB_AREA_NM)= UPPER(''`+ P_SUB_AREA_NM +`'') AND UPPER(INTERFACE_NM)=UPPER(''`+ P_INTERFACE_NM +`'') AND UPPER(OBJECT_LAYER)=UPPER(''`+ P_OBJECT_LAYER +`'') AND UPPER(RULE_TYPE)=UPPER(''ERROR''))`;
...
---------Updating non error records as N
...
----------Fetching non error record counts
...
----------Making Audit details entry for   Records with errors updated to Y
			
		var SQ_INSRT_AUDIT_LOG_Y = `Insert into COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__DATA_VLDTN_ADT_DTLS(BATCH_ID,RULE_ID,SUB_AREA_NM,INTERFACE_NM,OBJECT_LAYER,SP_NM,SP_INPUT_PARM,RECORD_COUNT,RULE_EXEC_STATUS,RULE_EXEC_DESC,OPRATION_TYPE,RULE_EXEC_START_TIME,RULE_EXEC_END_DATE,LAST_MODIFIED_DATE,LAST_MODIFIED_BY) values(`+ V_BATCH_ID +`,NULL,''`+ P_INTERFACE_NM +`'',''`+ P_INTERFACE_NM +`'',''`+ P_OBJECT_LAYER +`'',''SP_STG_UPDATE'',''`+ V_INPUT_PARAM +`'',''`+ V_ERROR_REC_CNT_Y +`'',''C'',''Records with errors updated to Y'',''U'',CURRENT_DATE(),CURRENT_DATE(),CURRENT_DATE(),''ETL_USER'')`;
...
-----------Making Audit details entry for Records with errors updated to N
		
		var SQ_INSRT_AUDIT_LOG_N = `Insert into COMETL_CONTROL_TEST_DB.COMETL_CONTROL_EMEA.PA__DATA_VLDTN_ADT_DTLS(BATCH_ID,RULE_ID,SUB_AREA_NM,INTERFACE_NM,OBJECT_LAYER,SP_NM,SP_INPUT_PARM,RECORD_COUNT,RULE_EXEC_STATUS,RULE_EXEC_DESC,OPRATION_TYPE,RULE_EXEC_START_TIME,RULE_EXEC_END_DATE,LAST_MODIFIED_DATE,LAST_MODIFIED_BY) values(`+ V_BATCH_ID +`,NULL,''`+ P_INTERFACE_NM +`'',''`+ P_INTERFACE_NM +`'',''`+ P_OBJECT_LAYER +`'',''SP_STG_UPDATE'',''`+  V_INPUT_PARAM +`'',''`+ V_ERROR_REC_CNT_N +`'',''C'',''Records with no errors updated to N'',''U'',CURRENT_DATE(),CURRENT_DATE(),CURRENT_DATE(),''ETL_USER'')`;

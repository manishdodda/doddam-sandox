CREATE OR REPLACE PROCEDURE k_database_name.k_schema_name.SP_k_project_name_AUDIT_LOG("P_INTERFACE_NM" VARCHAR(200), "P_TASK_NAME" VARCHAR(200), "P_SUB_AREA_NM" VARCHAR(200), "P_TASK_TYPE" VARCHAR(200), "P_EXECUTION_STATUS" VARCHAR(200), "P_EXCEP_ERR" VARCHAR(200))
...
-------Fetching active batch id from batch details
		 
	var V_FETCH_BATCH_SQ = `SELECT BATCH_ID,APPLICATION_ID FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME) = UPPER(''`+ P_SUB_AREA_NM +`'') AND LOAD_STATUS = ''R''`;
	
	var V_FETCH_BATCH_ST = snowflake.createStatement( {sqlText: V_FETCH_BATCH_SQ} ).execute();

--------------------If active batch found
...
-------Updating AUDIT_DTLS with parametered P_EXCEP_ERR in case of C and F

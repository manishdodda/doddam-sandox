CREATE OR REPLACE PROCEDURE k_database_name.k_schema_name.SP_k_project_name_BATCH_RUN_DTLS("P_APPLICATION_NAME" VARCHAR(200), "P_SUBJECT_AREA_NAME" VARCHAR(200), "P_STEP" VARCHAR(200))
...
-------Selecting Param value from parameter table
    	...
    	var V_SQ = `SELECT PARAM_VALUE FROM `+V_CONTROL_SCHEMA_NAME+`.k_project_name_PARAMETER_TABLE WHERE PARAM_NAME = ''CONTROL'' AND APPLICATION_ID = (SELECT APPLICATION_ID FROM `+V_CONTROL_SCHEMA_NAME+`.k_project_name_APPLICATION_DTLS WHERE UPPER(APPLICATION_NAME) = UPPER(''`+ P_APPLICATION_NAME +`''))`;
...
--------Selecting max batch value from batch table
		...
		var V_SQ1 = `SELECT BATCH_ID, LOAD_STATUS FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_BATCH_RUN_DTLS WHERE APPLICATION_ID = (SELECT APPLICATION_ID FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_APPLICATION_DTLS WHERE UPPER(APPLICATION_NAME) = UPPER(''`+ P_APPLICATION_NAME +`'')) AND BATCH_ID = (SELECT MAX (BATCH_ID) FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME) = UPPER(''`+ P_SUBJECT_AREA_NAME +`''))`;
...
-------Feching respective application id from application table 
		...
		var V_APPLICATION_ID_SQ = `SELECT APPLICATION_ID FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_APPLICATION_DTLS WHERE UPPER(APPLICATION_NAME)= UPPER(''`+ P_APPLICATION_NAME +`'')`;
...
----------Inserting new batch entry 
			...
			var V_SQ3 = `INSERT INTO `+V_DATABASE_SCHEMA_NAME+`.k_project_name_BATCH_RUN_DTLS (BATCH_ID,APPLICATION_ID,SUBJECT_AREA_NAME,EXECUTION_DATE,LOAD_START_TIME,LOAD_STATUS) VALUES (`+V_BATCH_ID+`,`+ V_APPLICATION_ID +`,''`+ P_SUBJECT_AREA_NAME +`'',CURRENT_DATE(),CURRENT_TIMESTAMP(),''R'')`;
...
// To create the sequence internally when ever the batch is inserted as in "R" state.
             ...
             var V_SQ6 = `CALL "SP_k_project_name_SEQUENCE_GEN" (''`+P_APPLICATION_NAME+`'',''OPEN_SEQ'')`;
...
-------If batch needs to be closed
	...
	if (P_STEP == ''BATCH_CLS'')
	{
		var V_SQ4 = `SELECT BATCH_ID FROM `+V_DATABASE_SCHEMA_NAME+`.k_project_name_BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME) = UPPER(''`+ P_SUBJECT_AREA_NAME +`'') AND LOAD_STATUS = ''R''`;
...
// To drop the sequence internally when ever the batch is closed as in "C" state.
            ...
            var V_SQ7 = `CALL "SP_k_project_name_SEQUENCE_GEN" (''`+P_APPLICATION_NAME+`'',''CLOSE_SEQ'')`;
...
------------Marking batch as Complete C
			...
			var V_SQ5 = `UPDATE `+V_DATABASE_SCHEMA_NAME+`.k_project_name_BATCH_RUN_DTLS SET LOAD_END_TIME = CURRENT_TIMESTAMP() ,LOAD_TIME_TAKEN_SECS = datediff (SECOND,LOAD_START_TIME, CURRENT_TIMESTAMP()), LOAD_STATUS = ''C'' WHERE UPPER(SUBJECT_AREA_NAME) = UPPER(''`+ P_SUBJECT_AREA_NAME +`'') AND LOAD_STATUS = ''R''AND BATCH_ID = `+ V_CURR_BATCH +``;

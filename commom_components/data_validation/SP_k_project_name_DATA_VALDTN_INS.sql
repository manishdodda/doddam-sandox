...
//     Fetching only those RULES which have IS_ACTIVE as Y
		...
		var V_SQ2 = `SELECT DISTINCT SRC_TABLE_SCHEMA, SRC_TABLENM, RUN_ID_FIELD FROM k_database_name.k_schema_name.k_project_name_DATA_VLDTN_RULE WHERE IS_ACTIVE = ''Y''
...
//     Deleting all error entries from error details for particular batch
        ...
        FROM k_database_name.k_schema_name.k_project_name_DATA_VLDTN_RULE WHERE IS_ACTIVE = ''Y'' AND 
		...
		var V_SQ_VW = `SELECT DISTINCT SRC_TABLENM, SRC_TABLE_SCHEMA
         FROM k_database_name.k_schema_name.k_project_name_ERROR_DTLS WHERE UPPER(INTERFACE_NM) = UPPER(''` + P_INTERFACE_NM + `'') AND UPPER(SUB_AREA_NM) = UPPER(''` + P_SUB_AREA_NM +`'')  AND BATCH_ID =` + V_BATCH_ID +`  AND UPPER(OBJECT_LAYER)= UPPER(''` + P_OBJECT_LAYER +`'')`;
			...
			while (V_ST_VW.next()){
				...
				var V_SQ_KEY = `SELECT DISTINCT UNIQUE_KEY FROM k_database_name.k_schema_name.k_project_name_DATA_VLDTN_RULE WHERE UPPER(SRC_TABLENM) = UPPER(''` + V_NM +`'') `;

...
----------Selecting active batch
		...
		var SQ_BATCH_ID = `SELECT BATCH_ID FROM SFDB.DEPLOY.DEMO_BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME)=UPPER(''`+ P_SUB_AREA_NM +`'') AND LOAD_STATUS=''R''`;
		//return SQ_BATCH_ID;
...
---------Updating error records as Y
		...
		var SQ_ERR_UPD_STATEMENT_Y = `UPDATE `+ V_TABLE_NM +` SET ERROR_FLAG =''Y'' WHERE `+ V_SRC_UNIQUE_KEY +` in (Select distinct SRC_UNIQUE_KEY from SFDB.DEPLOY.DEMO_ERROR_DTLS where BATCH_ID =`+ V_BATCH_ID +` AND UPPER(SUB_AREA_NM)= UPPER(''`+ P_SUB_AREA_NM +`'') AND UPPER(INTERFACE_NM)=UPPER(''`+ P_INTERFACE_NM +`'') AND UPPER(OBJECT_LAYER)=UPPER(''`+ P_OBJECT_LAYER +`'') AND UPPER(RULE_TYPE)=UPPER(''ERROR''))`;

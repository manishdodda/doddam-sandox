CREATE OR REPLACE PROCEDURE SFDB.DEPLOY.SP_DEMO_AUDIT_LOG("P_INTERFACE_NM" VARCHAR(200), "P_TASK_NAME" VARCHAR(200), "P_SUB_AREA_NM" VARCHAR(200), "P_TASK_TYPE" VARCHAR(200), "P_EXECUTION_STATUS" VARCHAR(200), "P_EXCEP_ERR" VARCHAR(200))
...
try 
{
    ...
    var V_CONTROL_SCHEMA_NAME = "DEPLOY";
	...
	var V_FETCH_BATCH_SQ = `SELECT BATCH_ID,APPLICATION_ID FROM `+V_DATABASE_SCHEMA_NAME+`.DEMO_BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME) = UPPER(''`+ P_SUB_AREA_NM +`'') AND LOAD_STATUS = ''R''`;

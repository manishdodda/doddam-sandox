CREATE OR REPLACE PROCEDURE SFDB.DEPLOY.SP_SFA_DATA_VALDTN_INS("P_SUB_AREA_NM" VARCHAR(16777216), "P_INTERFACE_NM" VARCHAR(16777216), "P_OBJECT_LAYER" VARCHAR(16777216), "P_DATABASE_NM" VARCHAR(16777216))
...
-----------Initiating audit sp
		...
		var V_AUDIT_INS_SQ = `CALL SFDB.DEPLOY.SP_SFA_AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_SFA_DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''Procedure'', ''R'',''NA'' )`; 
...
-----------Fetching Batch details
		...
		var V_SQ1 = `SELECT BATCH_ID FROM SFDB.DEPLOY.SFA_BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME)=UPPER(''`+ P_SUB_AREA_NM +`'') 
		AND LOAD_STATUS = ''R''`;
...
----------Deleting all error entries from error details for particular batch
		...
		var V_SQ4 = `DELETE FROM  SFDB.DEPLOY.SFA_ERROR_DTLS WHERE BATCH_ID=`+ V_BATCH_ID +` AND SUB_AREA_NM = ''` + P_SUB_AREA_NM +`''
...
---------Fetching error counts using rule sql in data validation
		...
		if(V_RUN_ID_FIELD==''BATCH_ID'')
		{
			...
			var V_SQ6 =`INSERT INTO SFDB.DEPLOY.SFA_ERROR_DTLS SELECT
...
-------------Fetching counts from error details table
		...
		var V_SQ7 =`SELECT COUNT(*) FROM SFDB.DEPLOY.SFA_ERROR_DTLS 
...
------------Comparing error details count vs counts from data validation having  rule_sql as where clause
		...
		var V_SQ_VW = `SELECT DISTINCT SRC_TABLENM, SRC_TABLE_SCHEMA
         FROM SFDB.DEPLOY.SFA_ERROR_DTLS WHERE UPPER(INTERFACE_NM) = UPPER(''` + P_INTERFACE_NM + `'') AND UPPER(SUB_AREA_NM) = UPPER(''` + P_SUB_AREA_NM +`'')  AND BATCH_ID =` + V_BATCH_ID +`  AND UPPER(OBJECT_LAYER)= UPPER(''` + P_OBJECT_LAYER +`'')`;
						v_return_value += "\\n V_SQ_VW: " + V_SQ_VW;
...
-------------Creating Error views				
				...
				SELECT distinct B.* , A.RULE_ID, A.RULE_TYPE, A.ERROR_MSG FROM SFDB.DEPLOY.SFA_ERROR_DTLS A
...
------------Calling audit sp	
		...
		var V_AUDIT_UPD_COMP_SQ = `CALL SFDB.DEPLOY.SP_SFA_AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_SFA_DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''PROCEDURE'', ''C'',''NA'' )`;
	...
	catch (err)  
	{
	  ...
	  var V_AUDIT_UPD_FAIL_SQ = `CALL SFDB.DEPLOY.SP_SFA_AUDIT_LOG(''`+P_INTERFACE_NM+`'',''SP_SFA_DATA_VALDTN_INS'',''`+P_SUB_AREA_NM+`'',''PROCEDURE'', ''F'',''`+output_return_value+`'')`;  

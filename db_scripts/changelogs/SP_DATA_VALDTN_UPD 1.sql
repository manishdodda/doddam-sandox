...
----------Selecting active batch
		...
		var SQ_BATCH_ID = `SELECT BATCH_ID FROM SFDB.DEPLOY.BATCH_RUN_DTLS WHERE UPPER(SUBJECT_AREA_NAME)=UPPER(''`+ P_SUB_AREA_NM +`'') AND LOAD_STATUS=''R''`;
		//return SQ_BATCH_ID;
...
---------Updating error records as Y
		...
		var SQ_ERR_UPD_STATEMENT_Y = `UPDATE `+ V_TABLE_NM +` SET ERROR_FLAG =''Y'' WHERE `+ V_SRC_UNIQUE_KEY +` in (Select distinct SRC_UNIQUE_KEY from SFDB.DEPLOY.ERROR_DTLS where BATCH_ID =`+ V_BATCH_ID +` AND UPPER(SUB_AREA_NM)= UPPER(''`+ P_SUB_AREA_NM +`'') AND UPPER(INTERFACE_NM)=UPPER(''`+ P_INTERFACE_NM +`'') AND UPPER(OBJECT_LAYER)=UPPER(''`+ P_OBJECT_LAYER +`'') AND UPPER(RULE_TYPE)=UPPER(''ERROR''))`;
...
-----------Making Audit details entry for Records with errors updated to N
		...
		var SQ_INSRT_AUDIT_LOG_N = `Insert into SFDB.DEPLOY.DATA_VLDTN_ADT_DTLS(BATCH_ID,RULE_ID,SUB_AREA_NM,INTERFACE_NM,OBJECT_LAYER,SP_NM,SP_INPUT_PARM,RECORD_COUNT,RULE_EXEC_STATUS,RULE_EXEC_DESC,OPRATION_TYPE,RULE_EXEC_START_TIME,RULE_EXEC_END_DATE,LAST_MODIFIED_DATE,LAST_MODIFIED_BY) values(`+ V_BATCH_ID +`,NULL,''`+ P_INTERFACE_NM +`'',''`+ P_INTERFACE_NM +`'',''`+ P_OBJECT_LAYER +`'',''SP_STG_UPDATE'',''`+  V_INPUT_PARAM +`'',''`+ V_ERROR_REC_CNT_N +`'',''C'',''Records with no errors updated to N'',''U'',CURRENT_DATE(),CURRENT_DATE(),CURRENT_DATE(),''devops'')`;
